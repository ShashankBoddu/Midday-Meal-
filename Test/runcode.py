    ...
    while True:
        try:
            ...
            if data:
                ...
                decoded_data = data.decode('utf-8').strip()  # Attempt to decode the data
                print(f"Decoded data: {decoded_data}")  # Print decoded data for debugging
                if decoded_data:
                    weight = decoded_data  # Update the global weight variable
                    print(f"Weight {weight}")
        except Exception as e:
            print(f"Error reading from UART: {e}")  # Handle all errors
...
def update_display():
    ...
    with lock:
        ...
        weight_label.config(text=f"Weight: {weight} kg")  # Update the label with the weight
...
def setup_video_capture():
    ...
    cap2 = reconnect_camera(2)  # Start second USB camera (camera index 1)
